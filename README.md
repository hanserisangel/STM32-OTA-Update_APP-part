...
#### 3.5.1 flash 掉电保护
掉电保护核心是保证记录 / 扇区的原子性和合法性，避免掉电导致数据损坏或进度丢失。为防止掉电导致进度记录损坏或丢失，本实现采用了**分阶段提交 + 双重校验**的保护机制：
- 分阶段提交：所有日志记录和扇区头均采用 “先写主体、后写提交标记” 的方式写入。只有当提交标记写入成功后，该记录才会被判定为有效。如果掉电发生在写入过程中，不完整的记录就会被直接忽略，不会影响下次启动时的进度恢复。
- 合法性校验：每次读取进度时，都会校验记录的魔数、提交状态和 CRC32 值，仅使用完全合法的记录。
#### 3.5.2 flash 磨损均衡
W25Q64 属于闪存介质，擦写次数有限（约 10 万次），代码通过**代际管理 + 扇区轮换 + 有序回收**实现磨损均衡。
- 代际管理：每个扇区头包含 generation 字段，初始为 0，每次擦除扇区并重新初始化时，generation 递增（旧扇区回收后变为新代际）；新增扇区时通过二分查找插入到有序数组，保证排序性。
- 扇区轮换：写入进度时优先使用最新代际的扇区（有剩余空间），而非固定扇区；当所有扇区无空间时，仅回收最旧代际的扇区（而非随机 / 固定扇区），擦除后重新初始化为 “最新代际”，实现扇区的循环使用，避免单个扇区被频繁擦写。
#### 3.5.3 flash 垃圾回收
垃圾回收核心是回收最旧扇区的无效数据，释放存储空间。当所有日志扇区都被写满时，自动触发垃圾回收流程，释放存储空间；选择代际号最旧的扇区作为回收目标；擦除目标扇区，并以新的代际号重新初始化，使其成为最新的可写扇区。
#### 3.5.4 flash 数据重建
设备上电初始化时，会自动扫描所有日志扇区，重建进度状态：
1.  按代际号从新到旧遍历扇区，反向扫描内部的日志记录。
2.  对每个记录进行合法性校验，筛选出当前任务的有效进度记录。
3.  找到序列号最大的有效记录，作为设备恢复运行后的最新下载进度。
4.  同时，通过二分查找算法维护扇区按 **generation（代际号）从小到大有序排列**，在新增/回收扇区时，**快速找到插入位置**，保证扇区列表始终有序，加速定位，减少重建时的扫描时间。
